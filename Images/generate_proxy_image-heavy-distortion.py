...
def apply_random_transform(img: Image.Image) -> Image.Image:
    ...
    # 1. Rotation — was (-15, 15), widen for more distortion
    angle = random.uniform(-30, 30)
    img = img.rotate(angle, expand=False, fillcolor=(255, 255, 255))

    # 2. Crop — was (0.80, 0.95), lower = more aggressive zoom/crop
    w, h = img.size
    crop_pct = random.uniform(0.60, 0.90)
    ...
    # 3. Brightness — was (0.7, 1.3), widen range
    brightness_factor = random.uniform(0.5, 1.6)
    img = ImageEnhance.Brightness(img).enhance(brightness_factor)

    # 4. Contrast — was (0.85, 1.15), widen range
    contrast_factor = random.uniform(0.7, 1.3)
    img = ImageEnhance.Contrast(img).enhance(contrast_factor)

    # 5. NEW: random horizontal flip (50% chance) — bigger structural change
    if random.random() < 0.2:
        img = img.transpose(Image.FLIP_LEFT_RIGHT)

    # 6. NEW: random blur (simulates motion blur/low quality capture)
